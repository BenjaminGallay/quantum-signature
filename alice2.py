...
STATE_WAITING_DELETE_CERT = "WAITING_DELETE_CERT"
STATE_DONE = "DONE"
...
def choose_private_key() -> list[tuple[tuple[str, str], tuple[str, str]]]:
    private_key = []
...
def verify_deletion_certificate(input_bits: str, private_key, certificate: str) -> bool:
    """Verify Bob's deletion proof for unused public keys."""
    expected_len = MSG_LENGTH * KEY_LENGTH

    if len(certificate) != expected_len:
        print(
            f"Alice: bad deletion certificate length {len(certificate)}, expected {expected_len}",
            flush=True,
        )
        return False
    ...
    for i, signed_bit_char in enumerate(input_bits):
        signed_bit = int(signed_bit_char)
        unused_bit = 1 - signed_bit

        x_unused = private_key[i][unused_bit][0]
        theta_unused = private_key[i][unused_bit][1]

        for k in range(KEY_LENGTH):
            measured = certificate[pos]

            if theta_unused[k] == "1":
                checked += 1
                if measured != x_unused[k]:
                    failures += 1

            pos += 1

    print(
        f"Alice: deletion proof checked {checked} X-basis positions, "
        f"failures={failures}",
        flush=True,
    )
...
async def run_alice(reader: StreamReader, writer: StreamWriter) -> None:
    ...
    while state != STATE_DONE:
        if state == STATE_WAITING_HI:
            ...
            state = STATE_WAITING_DELETE_CERT

        elif state == STATE_WAITING_DELETE_CERT:
            deletion_msg = (await reader.readline()).decode().strip()

            if not deletion_msg.startswith("DELETE_CERT:"):
                print(f"[{state}] Alice: expected DELETE_CERT, got {deletion_msg!r}")
                state = STATE_DONE
                continue

            certificate = deletion_msg[len("DELETE_CERT:") :]
            deletion_ok = verify_deletion_certificate(input_bits, private_key, certificate)

            if deletion_ok:
                print(f"[{state}] Alice: deletion proof ACCEPTED")
            else:
                print(f"[{state}] Alice: deletion proof REJECTED")
